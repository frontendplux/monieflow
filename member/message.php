...
// If we're the listing owner but no partner was specified (arrived via a
// direct/legacy link without ?partner=), auto-resolve the counterparty so
// escrow lookups — and the Release button — don't silently come up empty.
if ($user['uid'] === $listing['owner_uid'] && empty($partnerUid)) {
    // Prefer whoever currently has a locked escrow on this listing
    ...
    if ($autoEscrowRow) {
        ...
        // Otherwise fall back to the most recent chat counterparty on this listing
        $autoChat = $conn->prepare("SELECT CASE WHEN sender_uid = ? THEN receiver_uid ELSE sender_uid END AS partner_uid
                                     FROM p2p_chats WHERE listing_id = ? AND (sender_uid = ? OR receiver_uid = ?)
                                     ORDER BY created_at DESC LIMIT 1");
        $autoChat->bind_param("siss", $user['uid'], $listingId, $user['uid'], $user['uid']);
        $autoChat->execute();
        $autoChatRow = $autoChat->get_result()->fetch_assoc();

        if ($autoChatRow) {
            $partnerUid = $autoChatRow['partner_uid'];
        }
    }
}
...
//
// $partnerUid means different things depending on who's viewing: for the
// listing owner it's "the person I'm chatting with"; for a non-owner it's
// hardcoded to "the listing owner" — never the non-owner's own UID. Using
// $partnerUid directly in these formulas silently broke role detection for
// non-owner sellers (buy-type listings). $otherPartyUid fixes that: it's
// always "whoever isn't the listing owner in this specific trade."
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'fetch_messages') {
        ...
        // Fetch Escrow Status — matched to this listing and to the current user being
        // party to it (either side), not to an exact buyer+seller pair. Only one escrow
        // can be locked per listing at a time, so this is always the right one.
        $eStmt = $conn->prepare("SELECT * FROM p2p_escrows WHERE listing_id = ? AND (buyer_uid = ? OR seller_uid = ?) ORDER BY id DESC LIMIT 1");
        $eStmt->bind_param("iss", $listingId, $user['uid'], $user['uid']);
        $eStmt->execute();
        ...
        // Resolve the live status of every escrow referenced by an inline
        // 'escrow_init' bubble (not just the most recent one), so a Release
        // button/badge in the chat is always accurate even for older orders.
        $escrowStatuses = [];
        $escrowIdsInChat = [];
        foreach ($chats as $c) {
            if ($c['type'] === 'escrow_init' && !empty($c['payloads'])) {
                $p = json_decode($c['payloads'], true);
                if (!empty($p['escrow_id'])) {
                    $escrowIdsInChat[] = (int) $p['escrow_id'];
                }
            }
        }
        $escrowIdsInChat = array_unique($escrowIdsInChat);
        if (!empty($escrowIdsInChat)) {
            $placeholders = implode(',', array_fill(0, count($escrowIdsInChat), '?'));
            $types = str_repeat('i', count($escrowIdsInChat));
            $esStmt = $conn->prepare("SELECT id, status, deposit_pin FROM p2p_escrows WHERE id IN ($placeholders)");
            $esStmt->bind_param($types, ...$escrowIdsInChat);
            $esStmt->execute();
            foreach ($esStmt->get_result()->fetch_all(MYSQLI_ASSOC) as $er) {
                $escrowStatuses[(int) $er['id']] = $er;
            }
        }
        ...
        echo json_encode([
            ...
            'chats' => $chats, 
            'escrow' => $escrow, 
            'escrow_statuses' => (object) $escrowStatuses,
            'my_uid' => $user['uid'],
            'is_seller' => $isSeller,
            'is_buyer' => $isBuyer,
    ...
    if ($action === 'initiate_escrow') {
        ...
        try {
            ...
            // Lock the listing row and confirm it still has enough amount available.
            // Nobody can buy or sell more than the listing actually has on offer,
            // even though the funds themselves move through the wallet.
            $listingLockStmt = $conn->prepare("SELECT amount FROM p2p_listings WHERE id = ? AND status = 'active' FOR UPDATE");
            $listingLockStmt->bind_param("i", $listingId);
            $listingLockStmt->execute();
            $listingRow = $listingLockStmt->get_result()->fetch_assoc();

            if (!$listingRow) {
                throw new Exception("This listing is no longer active.");
            }

            if ($flowAmount > floatval($listingRow['amount'])) {
                throw new Exception("Amount exceeds what's available in this listing. Available: " . number_format($listingRow['amount'], 4) . " FLOW.");
            }
            ...
            // Deduct the same amount from the listing itself (atomic, re-checked
            // against the live row) so it can never be oversold/overbought across
            // multiple concurrent trades — this is what actually prevents the scam,
            // the wallet deduction alone doesn't touch the advertised amount.
            $deductListingStmt = $conn->prepare("UPDATE p2p_listings SET amount = amount - ? WHERE id = ? AND amount >= ?");
            $deductListingStmt->bind_param("did", $flowAmount, $listingId, $flowAmount);
            $deductListingStmt->execute();
            if ($deductListingStmt->affected_rows === 0) {
                throw new Exception("Listing no longer has enough amount available. Please refresh and try again.");
            }

            // If the listing is now fully spoken for, close it so it stops
            // appearing as an active offer for new buyers/sellers.
            $remainingStmt = $conn->prepare("SELECT amount FROM p2p_listings WHERE id = ?");
            $remainingStmt->bind_param("i", $listingId);
            $remainingStmt->execute();
            $remainingRow = $remainingStmt->get_result()->fetch_assoc();
            if ($remainingRow && floatval($remainingRow['amount']) <= 0) {
                $closeStmt = $conn->prepare("UPDATE p2p_listings SET status = 'completed' WHERE id = ?");
                $closeStmt->bind_param("i", $listingId);
                $closeStmt->execute();
            }
            ...
            $insE->execute();
            $escrowId = $conn->insert_id;

            // Full audit trail: log the seller's wallet debit into the transaction table
            $txSellerDesc = "P2P escrow locked — Listing #{$listingId}";
            $txSellerPayload = json_encode(['escrow_id' => $escrowId, 'listing_id' => $listingId, 'buyer_uid' => $buyerUid]);
            $txSeller = $conn->prepare("INSERT INTO transaction (uid, amount, type, description, payloads) VALUES (?, ?, 'debit', ?, ?)");
            $txSeller->bind_param("sdss", $sellerUid, $flowAmount, $txSellerDesc, $txSellerPayload);
            $txSeller->execute();

            // Post the escrow directly into the chat as its own message type, carrying
            // enough payload for the seller to hit "Release" right from the bubble.
            $sysMsg = "Fiat: ₦" . number_format($fiatAmount, 2) . "\nAmount: " . number_format($flowAmount, 4) . " FLOW locked in escrow.\nFunds stay locked until the seller confirms payment and releases them manually.";
            $escrowPayload = json_encode([
                'escrow_id' => $escrowId,
                'gross_amount' => $flowAmount,
            ...
            $sysStmt->bind_param("issss", $listingId, $user['uid'], $partnerUid, $sysMsg, $escrowPayload);
            $sysStmt->execute();
    ...
    if ($action === 'release_escrow') {
        ...
        try {
            ...
            $gasFee        = $grossAmount * GAS_FEE_PCT;        // 2.5% total — billed to the listing owner, not the trade
            $mfShare       = $grossAmount * MF_VALUE_PCT;       // 1.2% -> coin value
            $companyShare  = $grossAmount * COMPANY_PROFIT_PCT; // 1.3% -> company profit
            $netAmount     = $grossAmount;                      // Buyer receives the full traded amount — no fee taken from it

            // The gas fee is billed separately to the listing owner's wallet — never
            // deducted from what the buyer or seller actually agreed to trade.
            $feePayerUid = $listing['owner_uid'];

            $feeWalletStmt = $conn->prepare("SELECT balance FROM wallets WHERE uid = ? FOR UPDATE");
            $feeWalletStmt->bind_param("s", $feePayerUid);
            $feeWalletStmt->execute();
            $feeWallet = $feeWalletStmt->get_result()->fetch_assoc();

            if (!$feeWallet || floatval($feeWallet['balance']) < $gasFee) {
                throw new Exception("The listing owner's wallet doesn't have enough balance to cover the gas fee. Release cannot proceed.");
            }

            $deductFeeStmt = $conn->prepare("UPDATE wallets SET balance = balance - ? WHERE uid = ?");
            $deductFeeStmt->bind_param("ds", $gasFee, $feePayerUid);
            $deductFeeStmt->execute();

            $upE = $conn->prepare("UPDATE p2p_escrows SET status = 'released', deposit_pin = ? WHERE id = ?");
            $upE->bind_param("si", $pinCode, $escrowId);
            $upE->execute();

            // Buyer's voucher is funded with the full traded amount — the fee never touches it
            ...
            // Full audit trail: buyer's credit and the listing owner's fee debit
            $txBuyerDesc = "P2P escrow release — received " . number_format($netAmount, 4) . " FLOW (Listing #{$listingId})";
            $txBuyerPayload = json_encode(['escrow_id' => $escrowId, 'listing_id' => $listingId, 'pin_code' => $pinCode]);
            $txBuyer = $conn->prepare("INSERT INTO transaction (uid, amount, type, description, payloads) VALUES (?, ?, 'credit', ?, ?)");
            $txBuyer->bind_param("sdss", $escrow['buyer_uid'], $netAmount, $txBuyerDesc, $txBuyerPayload);
            $txBuyer->execute();

            $txFeeDesc = "P2P gas fee (2.5%) on escrow release — Listing #{$listingId}";
            $txFeePayload = json_encode(['escrow_id' => $escrowId, 'listing_id' => $listingId, 'mf_value_share' => $mfShare, 'company_share' => $companyShare]);
            $txFee = $conn->prepare("INSERT INTO transaction (uid, amount, type, description, payloads) VALUES (?, ?, 'debit', ?, ?)");
            $txFee->bind_param("sdss", $feePayerUid, $gasFee, $txFeeDesc, $txFeePayload);
            $txFee->execute();

            $sysMsg = "Escrow Released!\nAmount: " . number_format($netAmount, 4) . " FLOW (fee-free for both parties)\nDeposit Voucher Code: " . $pinCode;
            $payload = json_encode([
                'pin_code' => $pinCode,
                'gross_amount' => $grossAmount,
                'net_amount' => $netAmount,
                'gas_fee' => $gasFee,
                'fee_payer' => $feePayerUid,
            ...
            $conn->commit();
            echo json_encode(['status' => true, 'message' => 'Redemption code generated! Buyer receives the full ' . number_format($netAmount, 4) . ' FLOW — the 2.5% gas fee was billed to the listing owner separately.']);
        } catch (Exception $e) {
            $conn->rollback();
            echo json_encode(['status' => false, 'message' => $e->getMessage()]);
        }
    ...
    if ($action === 'send_proof') {
        ...
        // Must have a locked escrow open for this trade — matched by listing + this
        // user as the buyer, since only one escrow can be locked per listing at a time.
        $eStmt = $conn->prepare("SELECT * FROM p2p_escrows WHERE listing_id = ? AND buyer_uid = ? AND status = 'locked' ORDER BY id DESC LIMIT 1");
        $eStmt->bind_param("is", $listingId, $user['uid']);
        $eStmt->execute();
        $escrow = $eStmt->get_result()->fetch_assoc();
...
// Matched by listing + current user's involvement (either side), not an
// exact buyer/seller pair — only one escrow can be locked per listing at a time.
$curEStmt = $conn->prepare("SELECT * FROM p2p_escrows WHERE listing_id = ? AND (buyer_uid = ? OR seller_uid = ?) ORDER BY id DESC LIMIT 1");
$curEStmt->bind_param("iss", $listingId, $user['uid'], $user['uid']);
$curEStmt->execute();
...
<body>
        ...
        <div class="row g-3">
            ...
            <div class="col-lg-8">
                ...
                <div class="escrow-lock-banner mb-3">
                    ...
                    A 2.5% gas fee applies when funds are released — billed to the listing owner's own wallet, not to the buyer or the amount being traded. The buyer always receives the full agreed amount. 1.2% of the fee strengthens the MF coin's value, and 1.3% funds platform operations.
                </div>
                ...
                <?php if ($isSeller && $currentEscrow && $currentEscrow['status'] === 'locked'): ?>
                    <?php
                        ...
                        $rcOwnerLabel = ($listing['owner_uid'] === $user['uid']) ? 'your wallet' : "the listing owner's wallet";
                    ?>
                        ...
                        <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                            <div>
                                ...
                                <div class="small text-muted">
                                    Buyer Receives (full amount): <strong class="text-success"><?= number_format($rcGross, 4) ?> FLOW</strong> ·
                                    Gas Fee (2.5%, billed to <?= $rcOwnerLabel ?>): <strong><?= number_format($rcFee, 4) ?> FLOW</strong>
                                </div>
                            </div>
                ...
                <div class="card border-0 shadow-sm mb-3 bot-card p-3">
                    ...
                    <div id="botWorkflow" class="mt-3 d-none">
                        <hr class="my-2">
                        <div class="small text-muted mb-2"><i class="bi bi-box-seam me-1"></i> Available in this listing: <strong><?= number_format($listing['amount'], 4) ?> FLOW</strong></div>
                        ...
                        <div class="p-2 bg-white rounded border mb-3">
                            ...
                            <div class="small text-danger d-none mt-1" id="fiatOverLimit">Exceeds what's available in this listing.</div>
                        </div>
    ...
    <script>
        ...
        const listingAvailable = <?= floatval($listing['amount']) ?>;
        ...
        function calculateFlow() {
            ...
            const flowAmount = fiat > 0 ? fiat / unitRate : 0;
            document.getElementById('calculatedFlow').innerText = `${flowAmount.toFixed(4)} FLOW`;

            const overLimit = document.getElementById('fiatOverLimit');
            if (overLimit) {
                overLimit.classList.toggle('d-none', flowAmount <= listingAvailable);
            }
        }
        ...
        async function initiateEscrowOrder() {
            ...
            const flowAmount = parseFloat(fiatAmount) / unitRate;
            if (flowAmount > listingAvailable) {
                alert(`This listing only has ${listingAvailable.toFixed(4)} FLOW available. Please enter a smaller amount.`);
                return;
            }
        ...
        async function releaseEscrow(escrowId, btnEl) {
            if (!confirm('Are you sure payment has been received? The buyer will receive the full amount. A 2.5% gas fee will be billed separately to the listing owner\'s wallet (1.2% to MF coin value, 1.3% to platform).')) return;
