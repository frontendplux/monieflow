...
//         $likeStmt = $this->conn->prepare("
//             SELECT COUNT(*) AS cnt 
//             FROM likes 
//             WHERE feed_id=? AND parent_id=? AND type='comment'
//         ");
...
//                 SELECT COUNT(*) AS cnt 
//                 FROM likes 
//                 WHERE feed_id=? AND parent_id=? AND type='reply'
//             ");
...
//             $replyCountStmt = $this->conn->prepare("
//                 SELECT COUNT(*) AS cnt 
//                 FROM comments 
//                 WHERE feed_id=? AND parent_id=?
//             ");
...
//         // Total reply count for this comment (all replies, not just limited 3)
//         $replyCountStmt = $this->conn->prepare("
//             SELECT COUNT(*) AS cnt 
//             FROM comments 
//             WHERE feed_id=? AND parent_id=?
...
//         // Total likes on replies under this comment
//         $replyLikesStmt = $this->conn->prepare("
//             SELECT COUNT(*) AS cnt 
//             FROM likes 
//             WHERE feed_id=? 
//               AND parent_id IN (SELECT id FROM comments WHERE parent_id=? AND feed_id=?)
//               AND type='reply'
//         ");
//         $replyLikesStmt->bind_param('iii', $feed_id, $c['comment_id'], $feed_id);
//         $replyLikesStmt->execute();
//         $c['reply_like_count'] = $replyLikesStmt->get_result()->fetch_assoc()['cnt'];

//         $c['replies'] = $replies;
//     }

//     // Feed-level counts
//     // Total likes across feed, comments, and replies
//     $sqlLikes = "SELECT COUNT(*) AS like_count FROM likes WHERE feed_id = ?";
...
//     $sqlCommentCount = "SELECT COUNT(*) AS comment_count FROM comments WHERE feed_id = ?";
...
public function feedWithComments($feed_id = 0) {
    ...
    foreach ($comments as &$c) {
        ...
        // Like count for this comment using comment_id
        $likeStmt = $this->conn->prepare("
            SELECT COUNT(*) AS cnt 
            FROM likes 
        ...
        foreach ($replies as &$r) {
            ...
            // Like count for reply using comment_id
            $likeStmt = $this->conn->prepare("
                SELECT COUNT(*) AS cnt 
                FROM likes 
                WHERE feed_id=? AND comment_id=?
...
public function sendComment($feed_id, $text, $parent_id = 0, $status = 'feeds') {
    ...
    $stmt = $this->conn->prepare(
        "INSERT INTO comments (feed_id, user_id, comment_id, parent_id, data, status, type) 
         VALUES (?, ?, ?, ?, ?, 'comment')"
    );
